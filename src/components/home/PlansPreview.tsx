...
export default function PlansPreview({ plans, whatsappNumber }: PlansPreviewProps) {
  ...
  return (
    <section id="plans" className="py-16 md:py-[140px] bg-brand-off-white px-6 md:px-12">
      <div className="max-w-[1280px] mx-auto">
        <FadeIn>
          ...
          <h2 className="font-serif font-bold text-[28px] md:text-[56px] text-brand-black mb-4 text-center">Pick Your Plan. Own Your Market.</h2>
          ...
          {plans.map((plan, i) => {
            ...
            return (
              <FadeIn key={plan.id} delay={i * 0.1}>
                <div className={`group rounded-[20px] p-6 md:p-[48px] border relative bg-brand-white border-brand-border hover:bg-brand-black transition-colors duration-300 ${isPro ? 'lg:-translate-y-4 shadow-sm' : ''}`}>
                  {plan.isPopular && (
